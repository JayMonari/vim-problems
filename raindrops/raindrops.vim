...
function! Raindrops(number) abort
  let sound = ''
  if a:number % 3 == 0 | let sound .= 'Pling' | endif
  if a:number % 5 == 0 | let sound .= 'Plang' | endif
  if a:number % 7 == 0 | let sound .= 'Plong' | endif

  return strlen(l:sound) == 0 ? '' . a:number : l:sound
endfunction
